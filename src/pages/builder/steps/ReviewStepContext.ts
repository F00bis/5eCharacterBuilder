...
function calculateAsiBonus(draft: PartialCharacterDraft): Record<Ability, number> {
  const bonus: Record<Ability, number> = { strength: 0, dexterity: 0, constitution: 0, intelligence: 0, wisdom: 0, charisma: 0 };
  const featChoices = draft.featChoices;
  if (!featChoices) return bonus;

  for (const choice of featChoices) {
    if (choice.type !== 'asi' || !choice.asiBonuses) continue;
    for (const asiBonus of choice.asiBonuses) {
      bonus[asiBonus.ability] += asiBonus.amount;
    }
  }
...
export function calculateFinalAbilityScores(draft: PartialCharacterDraft): Record<Ability, number> {
  ...
  const asiBonus = calculateAsiBonus(draft);
  ...
  for (const ability of ABILITY_ORDER) {
    ...
    result[ability] = baseScore + raceBonus[ability] + asiBonus[ability] + featBonus + equipmentBonus;
  }
...
export function useReviewStep() {
  ...
  const finalCharacter = useMemo<Character>(() => {
    ...
    const acBreakdown = getArmorClassForDraft({ ...draftWithChoices, abilityScores });
